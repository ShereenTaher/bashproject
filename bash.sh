  ...
  if [  -f "$filename" ];then
     ...
     read -p "enter number of fields to update them:" fieldsNum;
		#  awk -F: '{print $0}'  $filename
		 awk -F":" '$'$fieldnum' == '$idvalue' { print $0}' $filename
		 #       echo $row
     for (( i = 0; i < $fieldsNum; i++ ));
      do
      read -p "enter old feild to update:" oldField;
       read -p "enter new feild:" newField;
		...
		;;
	 	selectsomecol_undercondition )
				...
				for i in "${fieldNum[@]}"
							...
							awk -F: '$'$colConditionNum'=='$valueCondition' {print '$x'}' $filename
		  break
		;;
